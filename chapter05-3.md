...
### 결론
  ...
  - 아키텍처 경계가 언제, 어디에 존재해야 할지, 그리고 그 경계를 완벽하게 구현할지, 부분 구현할지 결정하는 일 또한 아키텍트의 역할이다.
  
  
# 25장 계층과 경계
### 서론
  - 시스템이 세 가지 컴포넌트(UI, 업무 규칙, 데이터베이스)로만 구성된다 생각하기 쉽지만 대다수 시스템에서 컴포넌트 개수는 훨씬 많다.
  
### 움퍼스 사냥 게임
  - <img width="377" alt="1" src="https://user-images.githubusercontent.com/7076334/99408136-3ab5dc80-2933-11eb-9c37-46971553265b.png">
  
    - 요건 : 텍스트 기반 UI는 그대로 유지하고, 게임 규칙과 UI를 분리해서 제품을 여러 시장에 다양한 언어로 발매하고 싶다.
      - 소스 코드 의존성을 적절히 관리하면 UI 컴포넌트가 어떤 언어를 사용하더라도 게임 규칙을 재사용할 수 있다.
      - 게임 규칙은 어떤 종류의 언어가 사용되는지 몰라도 된다.
    
  - <img width="515" alt="2" src="https://user-images.githubusercontent.com/7076334/99408345-7a7cc400-2933-11eb-8c14-222d04d97784.png">
  
    - 요건 : 게임의 상태를 영속적인 저장소에 유지하고 싶다.
      - 게임 규칙이 다양한 종류의 데이터 저장소에 대해 알지 않기를 원한다.
      - 의존성 규칙을 준수할 수 있도록 의존성이 적절한 방ㅇ향을 가리키게 만들어야 한다.
      
### 클린 아키텍처?
  - 위의 예제가 아키텍처 경계를 모두 발견한 것일까?
    - UI에서 언어 뿐만 아니라 텍스트를 주고받는 메커니즘을 다양하게 할수도 있다. (클라이언트)
    - 언어를 통신 메커니즘으로 부터 격리하는 API를 생성할 수도 있다.
    
    - ![3](https://user-images.githubusercontent.com/7076334/99409189-743b1780-2934-11eb-93da-24505b8c2b2d.png)

      - 점선으로 된 테두리는 API를 정의하는 추상 컴포넌트를 가리키며, 이를 구현하는 구현 컴포넌트가 있다.
      - 게임규칙은 GameRules가 정의하고 Language가 구현하는 API를 이용하여 Language와 통신한다.
      - 언어는 Language가 정의하고 TextDelivery가 구현하는 API를 이용하여 TextDelivery와 통신한다.
      - 다형적 Bondary를 얘기하는데 이부분은 좀 이해가 안됨. (순환 참조 아닌가?)
      
    - <img width="280" alt="4" src="https://user-images.githubusercontent.com/7076334/99410346-b7e25100-2935-11eb-893f-60cf95e7fdf4.png">
    
      - 구현 클래스를 제거하고 순전히 API 컴포넌트만 집중하면 다이어그램을 단순화 할 수 있다.
      - 다이어그램은 모든 화살표가 위를 향하도록 맞춰져 있다. (저수준 -> 고수준)
        - GameRules는 최상위 수준의 정책이기 때문에 적절한 배치이다.
      - 정보의 흐름으로 생각해 보면
        - 입력 : 사용자입력 -> TextDelivery -> Language -> Gamerules(최상위) -> DataStorage
        - 출력 : GameRules -> Language -> TextDelivery
      - 이 구성은 데이터를 두개의 흐름으로 효과적으로 분리한다.
        - 왼쪽의 흐름은 사용자와의 통신
        - 오른쪽의 흐름은 데이터 영속성
        - 두 흐름은 상단의 GameRules에서 서로 만나며, GameRules는 데이터에 대한 최종적인 처리기가 된다.
        
### 흐름 횡단하기
  - <img width="406" alt="5" src="https://user-images.githubusercontent.com/7076334/99411486-fdebe480-2936-11eb-8f46-50ad2ab6009a.png">
  
    - 네트워크 컴포넌트가 추가되면서 세 개의 흐름으로 분리시켰다.
    - 시스템이 복잡해질수록 컴포넌트 구조는 더 많은 흐름으로 분리될 것이다.
    
### 흐름 분리하기
  - <img width="405" alt="6" src="https://user-images.githubusercontent.com/7076334/99411830-5e7b2180-2937-11eb-80b9-75e12aa52e0e.png">
    - 현실은 단순하지 않고 좀더 복잡하다.
    - 게임 규칙 중 일부는 지도와 관련된 메커니즘을 처리하고 이 규칙은 동굴관련 정보를 알고 있다.
    - 하지만 이보다 더 높은 수준에는 또 다른 정책 집합이 존재한다.
      - ex) 플레이어 생명력, 퀘스트 해결 비용과 보상등의 정책
    - 고수준 정책에서는 플레이어의 상태를 관리한다. 그리고 게임 종료 후, 플레이어의 승리 여부도 같이 결정한다.
    
  - 대규모의 플레이어가 동시에 플레이할 수 있는 움퍼스 사냥 게임 (그림 25.7)
    - MoveManagement는 플레이어의 컴퓨터(클라이언트)에서 직접 처리
    - PlayerManagement는 서버에서 처리
    - PlayerManagement는 접속된 모든 MoveManagement 컴포넌트에 마이크로서비스 API를 제공한다.
      - 둘 사이에는 완벽한 형태의 아키텍처 경계가 존재한다.

### 결론
  - 아키텍처 경계는 어디에나 존재할 수 있다.
  - 아키텍트로서 아키텍처 경계가 언제 필요한지 신중하게 파악해야하고 이러한 경계를 제대로 구현하려면 비용이 많이 든다.
    - 그렇다고 경계를 무시한다면 나중에 추가하는 비용도 크다.
  - 추상화가 필요하리라고 미리 예측하면 안된다.
    - 오버 엔지니어링이 언더 엔지니어링보다 나쁠때가 훨씬 많다.
  - 어디에 아키텍처 경계를 둬야 할지, 완벽하게 구현할 경계는 무엇인지와 부분적으로 구현할 경계와 무시할 경계는 무엇인지 결정해야 된다.
  - 초반에는 쉽게 결정할 수 없고 지켜보다가 시스템이 발전함에 따라 주의를 기울여야 한다.
