...
- SAP:안정된 추상화 원칙
  ...
  - **주게열**
    - 안정성(I)과 추상화 정도(A) 사이의 관계를 정의
      ...
      - A/I 그래프 상에서 컴포넌트가 위치할 수 있는 합리적인 지점을 찾기 위해 컴포넌트가 절대로 위치해서는 안되는 영역(배제할 구역)을 찾는 방식으로 추론할 수 있다.
    ...
    - **고통의 구역**
      ...
      - 변동성이 없는 컴포넌트는 (0,0) 구역에 위치했더라도 해롭지 않다. (변동될 가능성이 거의 없기 때문에...)
    ...
    - **쓸모없는 구역**
      ...
      - 이러한 엔티티는 누구도 구현하지 않은 채 남겨진 추상 클래스인 경우가 많다.
        - 쓸모 없는 엔티티가 존재한다는 사실은 바람직한 상황이 아님은 분명하다.
      
    - **배제 구역 벗어나기**
      - 변동성이 큰 컴포넌트 대부분은 (0,0), (1,1) 두 배제 구역으로부터 가능한 멀리 떨어뜨려야 한다.
        - 배제 구역으로부터 머릴 떨어진 점의 궤적은 (1,0)과 (0,1)을 잇는 선분이고 이를 주계열 이라고 부른다.
    ...
    - **주계열과의 거리**
      - D : 거리. D=|A+I-1|. 이 지표의 유효범위는 [0,1]이다. 
      - D가 0이면 컴포넌트가 주계열 바로 위에 위치한다는 뜻이며, 1이면 주계열로부터 가장 멀리 위치한다는 뜻이다.
      - D 값이 0에 가깝지 않은 컴포넌트가 있다면 해당 컴포넌트는 재검토한 후 재구성할 수 있다.
      
- 결론
  - 의존성 관리 지표는 설계의 의존성과 추상화 정도가 내가 '훌륭한' 패턴이라고 생각하는 수준에 얼마나 잘 부합하는지를 측정한다.
  - 지표는 불완전하지만 이들 지표로부터 무언가 유용한 것을 찾을 수 있기를 바란다.
