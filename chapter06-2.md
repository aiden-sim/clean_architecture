...
- 궁금증
  - 컴포넌트 분리를 패키지로 처리해야 될까 모듈로 처리해야 될까?
    - 답 : 단일 jar 파일 형태기 때문에 모듈 형태로 분리해야 될듯
...
- 수평적 계층화(계층 기반 패키지)와 수직적 계층화(기능 기반 패키지)는 둘다 차선책이다.
...
- 컴포넌트 기반 패키지 접근법의 이점
  - 주문과 관련된 무언가를 코딩해야 할때 오직 한 곳, OrdersComponent만 둘러보면 된다.
    - 내부에서 관심사의 분리는 여전히 유효
    - 업무 로직은 데이터 영속성과 분리되어 있음
      - MSA, SOA 때 얻는 이점과 유사한데 큰 차이는 결합 분리 모드
      - 모노리틱에서 컴포넌트 잘 정의하면 마이크로 서비스로 가는 발판으로 삼을 수 있음
    
### 구현 세부사항엔 항상 문제가 있다.
- 위 네가지 접근법이 코드를 조직화하는데 서로 다른 방식처럼 보이지만, 세부사항을 잘못 구현하면 이러한 견해도 아주 빠르게 흐트러지기 시작한다.
- 자바의 public
  - 우리는 public 키워드를 아무런 고민 없이 본능처럼 사용하는 것처럼 보인다.
  - 코드 베이스에서 지향한 아키텍처 스타일과도 아무런 관련이 없다.
  - 모든 타입에서 public 지시자를 사용한다는 건 캡슐화 관련 이점을 활용하지 않겠다는 뜻
  - 결국 당신이 지향하는 아키텍처 스타일을 위반하게 될 것이다.
  
### 조직화 vs 캡슐화
- 조직화
  - 모든 타입을 public으로 지정한다면, 패키지는 단순히 조직화를 위한 메커니즘(폴더)으로 전략하여 캡슐화를 위한 메커니즘이 될 수 없다.
  - 패키지를 무시해 버리면 최종적으로 어떤 아키텍처 스타일로 만들려고 하는지 아무런 의미가 없음
    - 앞의 네 가지 아키텍처 접근법은 본질적으로 완전히 같아짐

![34 7](https://user-images.githubusercontent.com/7076334/102073556-d2312100-3e46-11eb-9d2b-c0fbddc4bfc3.png)

- 개념적으로 이 접근법들은 매우 다르지만, 구문적으로 완전히 똑같음
- 자바의 접근 지시자는 완벽하지 않음
  - 접근성 제한 방식을 패키지의 상하위 관계대로 만들기는 불가능
  - 우리가 만든 계층은 그저 패키지의 이름과 디스크 상의 디스크 상의 디렉터리 구조에만 반영
- 그렇다고 무시하면 사서 고생하는 길이다.


![34 8](https://user-images.githubusercontent.com/7076334/102073565-d3fae480-3e46-11eb-8dc1-30e70e25ddf6.png)

- 계층 기반 패키지
  - OrdersService, OrdersRepository 인터페이스는 외부 패키지의 클래스로부터 의존성이 존재하므로 public으로 선언
  - 구현 클래스(OrderServiceImpl, JdbcOrdersRepositry)는 더 제한적으로 선언가능 (protected)
- 기능 기반 패키지
  - OrdersController가 패키지로 들어올 수 있는 유일한 통로 이므로(public) 나머지는 모두 패키지 protected로 지정 가능
  - 주의점은 컨트롤러를 통하지 않으면 주문 관련 정보에 접근할 수 없다. (바람직할 때도 있고 아닐때도 있고)  
- 포트와 어댑터
  - OrdersService와 Orders 인터페이스는 외부로부터 들어오는 의존성을 가지므로 public을 지정
  - 구현 클래스는 패키지 protected로 지정하며, 런타임에 의존성을 주입할 수 있음  
- 컴포넌트 기반 패키지
  - 컨트롤러에서 OrdersComponent 인터페이스로 향하는 의존성을 가지며, 그 외의 모든것은 패키지 protected로 지정할 수 있음
  - public 타입이 적을수록 필요한 의존성의 수도 적어짐
  - 컴파일러의 도움을 받아서 '컴포넌트 기반 패키지' 아키텍처 접근법을 강제할 수 있음
  
- 위의 설명들은 모노리틱 애플리케이션에 대한 것이다.
- 이러한 애플리케이션을 구축 중이면 아키텍처 원칙을 강제할 때 자기 규율이나 컴파일 후처리 도구를 이용하지 말고, 반드시 컴파일러에 의지할 것을 권장

### 다른 결합 분리 모드
- 프로그래밍 언어가 제공하는 방법 외에도 소스 코드 의존성을 분리하는 방법은 존재 (모듈)
  - ex) OSGI 같은 모듈 프레임워크, 자바9 모듈 시스템
  - 모듈 시스템을 제대로 사용하면 public 타입과 외부에 공표할 타입을 분리
- 소스 코드 수준에서 의존성을 분리하는 방법
  - 서로 다른 소스 코드 트리로 분리
  - ex) 포트와 어댑터
    - 업무와 도메인용 소스 코드(기술이나 프레임워크와는 독립적) : OrderService, OrderServiceImpl, Orders
    - 웹용 소스 코드 : OrdersController
    - 데이터 영속성용 소스 코드 : JdbcOrdersRepository
      - 웹용 소스 코드와 데이터 영속성용 소스 코드는 업무와 도메인 코드에 대해 컴파일 시점에 의존성을 갖음
      - 업무와 도메인 코드 자체는 웹이나, 데이터 코드에 대해서는 아무것도 알지 못함
  - 구현 관점에서 분리하려면 빌드 도구(maven, gradle)를 사용해서 모듈이나 프로젝트가 서로 분리되도록 구성
  - 소스 코드를 이처럼 나누면 성능, 복잡성, 유지보수 문제가 생길 수 있음

![34 9](https://user-images.githubusercontent.com/7076334/102076154-a31cae80-3e4a-11eb-9bbd-18bf5b4dd1db.png)

- 포트와 어댑터 접근법은 이보다 간단하게 소스 코드 트리 두개만 만들 수 있다
  - 도메인 코드(내부)
  - 인프라 코드(외부)
  
- 위 다이어그램에서 인프라는 도메인에 대해 컴파일 시점에 의존성을 가진다.
- 포트와 어댑터에 대한 페리페리크 안티 패턴
  - 페리페리크란? 프랑스 파리에 있는 원형 순환도로인데 이 도로를 이용하면 파리 시내에 진입하지 않고도 파리 전체를 다 돌 수 있음
  - 인프라 코드를 단일 소스 코드에 모두 모아둔다는 것은 애플리케이션에서 특정 영역에 있는 인프라 코드가 애플리케이션의 다른 영역에 있는 코드를 직접 호출 가능
    - 도메인을 통하지 않고 말이다.

### 결론 : 빠져 있는 조언
- 최적의 설계를 꾀했더라도, 구현 전략에 얽힌 복잡함을 고려하지 않으면 설계가 순식간에 망가질 수 있다.
- 설계를 어떻게 해야만 원하는 코드 구조로 매핑할 수 있을지, 그 코드를 어떻게 조직화 할지, 런타임과 컴파일 타임에 어떤 결합 분리 모드를 적용할지 고민
- 선택된 아키텍처 스타일을 강제하는데 컴파일러의 도움을 받을 수 있을지 고민, 데이터 모델과 같은 다른 영역에 결합되지 않도록 주의
